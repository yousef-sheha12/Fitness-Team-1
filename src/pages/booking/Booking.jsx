...
import { scheduleBooking } from "@/lib/api/booking.api";
...
const Booking = () => {
  ...
  const sessionDate = dateParam
    ? new Date(dateParam).toISOString().replace("T", " ").split(".")[0]
    : null;
  ...
  const handleBookNow = async () => {
    ...
    if (selectedMethod === "paypal" || selectedMethod === "vodafone") {
      try {
        const response = await scheduleBooking({
          trainer_package_id: trainerId,
          sessions: sessionDate
            ? [`${sessionDate.split(" ")[0]} ${timeParam || "09:00:00"}`]
            : [],
